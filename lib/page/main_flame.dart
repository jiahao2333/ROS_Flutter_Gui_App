...
class MainFlame extends FlameGame {
  ...
  Future<void> onLoad() async {
    ...
    _pointCloudComponent =
        PointCloudComponent(pointList: [], map: rosChannel!.map.value);

    _globalPathComponent = PathComponent(pointList: [], color: Colors.blue);
    _globalPathComponent.priority = 149;

    _localPathComponent = PathComponent(pointList: [], color: Colors.green);
    _localPathComponent.priority = 151;

    _tracePathComponent = PathComponent(pointList: [], color: Colors.yellow);
  ...
  void _setupRosListeners() {
    ...
    rosChannel!.laserPointData.addListener(() {
      if (rosChannel!.map_.value == null ||
          rosChannel!.map_.value.mapConfig.resolution <= 0 ||
          rosChannel!.map_.value.height() <= 0) return;
      ...
      for (int i = 0; i < laserPoints.laserPoseBaseLink.length; i++) {
        final laserPointMap = absoluteSum(
            robotPose,
            RobotPose(laserPoints.laserPoseBaseLink[i].x,
                laserPoints.laserPoseBaseLink[i].y, 0));
        vm.Vector2 laserPointScene = rosChannel!.map_.value
            .xy2idx(vm.Vector2(laserPointMap.x, laserPointMap.y));
  ...
  bool onScroll(double delta, Vector2 position) {
    ...
    final newZoom =
        (camera.viewfinder.zoom + zoomChange).clamp(minScale, maxScale);
  ...
  void _updateTopologyLayers() {
    ...
    print(
        '更新在线拓扑图层: ${topologyMap.points.length} 个点, ${topologyMap.routes.length} 条路径 离线导航点数量: ${offLineNavPoints.length}');
  ...
  void _setupLayerListeners() {
    ...
    // Add specific listeners for heavy layers to manage subscriptions
    globalState.getLayerState('showLaser').addListener(() {
      if (globalState.isLayerVisible('showLaser')) {
        rosChannel!.subscribeLaser();
      } else {
        rosChannel!.unsubscribeLaser();
      }
    });

    globalState.getLayerState('showPointCloud').addListener(() {
      if (globalState.isLayerVisible('showPointCloud')) {
        rosChannel!.subscribePointCloud();
      } else {
        rosChannel!.unsubscribePointCloud();
      }
    });

    // Costmaps usually go together in recent changes, but we split logic if needed.
    // Here we treat them as a group for subscription as per RosChannel change,
    // or we can just subscribe if EITHER is visible.
    // Let's bind 'showGlobalCostmap' and 'showLocalCostmap' to the subscribeCostmap helper.
    // Checks if ANY costmap is visible.
    final costmapListener = () {
      bool anyCostmapVisible =
          globalState.isLayerVisible('showGlobalCostmap') ||
              globalState.isLayerVisible('showLocalCostmap');
      if (anyCostmapVisible) {
        // Safe to call multiple times as RosBridgePlayer handles it
        rosChannel!.subscribeCostmap();
      } else {
        rosChannel!.unsubscribeCostmap();
      }
    };
    globalState.getLayerState('showGlobalCostmap').addListener(costmapListener);
    globalState.getLayerState('showLocalCostmap').addListener(costmapListener);

    globalState.getLayerState('showRobotFootprint').addListener(() {
      if (globalState.isLayerVisible('showRobotFootprint')) {
        rosChannel!.subscribeFootprint();
      } else {
        rosChannel!.unsubscribeFootprint();
      }
    });
  ...
  void _initializeLayerComponents() {
    ...
    // Initialize subscriptions based on initial state
    if (globalState.isLayerVisible('showLaser')) {
      rosChannel!.subscribeLaser();
    }
    if (globalState.isLayerVisible('showPointCloud')) {
    ...
    if (globalState.isLayerVisible('showGlobalCostmap') ||
        globalState.isLayerVisible('showLocalCostmap')) {
      rosChannel!.subscribeCostmap();
    }
    if (globalState.isLayerVisible('showRobotFootprint')) {
